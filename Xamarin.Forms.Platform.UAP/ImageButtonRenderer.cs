...
namespace Xamarin.Forms.Platform.UWP
{
	public class ImageButtonRenderer : ViewRenderer<ImageButton, FormsButton>, IImageVisualElementRenderer
	{
		...
		public override SizeRequest GetDesiredSize(double widthConstraint, double heightConstraint)
		{
			...
			var result = new Size
			{
				Width = ((BitmapSource)_image.Source).PixelWidth + Element.Padding.Left + Element.Padding.Right,
				Height = ((BitmapSource)_image.Source).PixelHeight + Element.Padding.Top + Element.Padding.Bottom
			};
		...
		void UpdatePadding()
		{
			Debug.WriteLine($"{((BitmapSource)_image.Source).PixelWidth} {((BitmapSource)_image.Source).PixelHeight} {_image.Width} {_image.Height} {_image.ActualWidth} {_image.ActualHeight} {Control.ActualWidth} {_formsButton.ActualWidth}");

			_image.Margin = new WThickness(0);

			if (Math.Floor(_image.ActualWidth - _formsButton.ActualWidth) > 0)
			{
